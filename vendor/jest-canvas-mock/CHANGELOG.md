...
When each `CanvasRenderingContext2D` object is created, all the methods are properly mocked with the `vi.fn()` method, and bound to the instance. It's still possible to verify that a function was called on the context. The main difference now is that the methods actually perform runtime checks on the passed parameters.
